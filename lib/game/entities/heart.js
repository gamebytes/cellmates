		...
		update: function() {
			if(!this.dying) {
				...
				if(this.releaseTimer.delta() > 0 && this.cellCount > 0) {
					...
					var cell = ig.game.spawnEntity(EntityCell, this.center.x + this.size.x / 2, this.center.y, {
						red: Math.random() < 0.75
					});

					cell.vel.x += this.heartRate.map(0, this.dangerZone, 0, cell.vel.x * 2);
					console.log(cell.vel.x);
				}
