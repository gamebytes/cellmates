...
'impact.game', 'plugins.observable', 'game.scenes.title', 'game.scenes.level', 'game.scenes.game-over').defines(function() {
	MainGame = ig.Game.extend({
    	...
    	var mappings = [[ gamepad.dpadUp, ig.KEY.UP_ARROW ],
                    ...
                    [ gamepad.faceButton0, ig.KEY.M ]];

			ig.show('dpadLeft', gamepad.dpadLeft);
			ig.show('dpadUp', gamepad.dpadUp);
			ig.show('dpadDown', gamepad.dpadDown);
			ig.show('dpadRight', gamepad.dpadRight);
